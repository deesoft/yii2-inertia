...
trait MergesProps
{
    ...
    {
        $this->matchOn = is_null($matchOn) ? [] : (is_array($matchOn) ? $matchOn : [$matchOn]);
     ...
     * @return static
     */
    public function prepend(bool|string|array $path = true, $matchOn = null)
    {
        if(is_bool($path)){
            $this->append = !$path;
            ...
            foreach($path as $key => $value){
                is_numeric($key) ? $this->prepend($value) : $this->prepend($key, $value);
            }
        }
     ...
     * @return static
     */
    public function append(bool|string|array $path = true, $matchOn = null)
    {
        if(is_bool($path)){
            $this->append = $path;
        }elseif(is_string($path)){
            ...
            foreach($path as $key => $value){
                is_numeric($key) ? $this->append($value) : $this->append($key, $value);
            }
        }
